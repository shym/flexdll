...
let new_cmdline () =
  ...
  | `MSVC | `MSVCARM64 | `MSVC64 | `LIGHTLD -> true
...
let run_command cmdline cmd =
  let cmd_quiet =
    ...
    | `MSVC | `MSVC64 | `MSVCARM64 when !verbose < 1 -> cmd ^ " >NUL"
    | _ -> cmd
  in
...
let main () =
  ...
  use_cygpath :=
    begin
      ...
      | (`MSVC|`MSVCARM64|`MSVC64|`LIGHTLD), `None -> false
    end;
